...
export interface WorkspaceMenuState {
  ...
  readonly isWorkspaceCollapsed: (workspaceId: string) => boolean;
...
export function useWorkspaceMenu(params: UseWorkspaceMenuParams): WorkspaceMenuState {
  ...
  const removeWorkspace = (workspace: WorkspaceRecord) => {
    const confirmed = window.confirm(`Remove ${workspace.name} from Alamelu Pi? This will not delete any files.`);
    setWorkspaceMenuId(null);
    setWorkspaceRenameId(null);
    if (!confirmed || !api) {
      return;
  ...
  const isWorkspaceCollapsed = (workspaceId: string) => {
    return collapsedWorkspaces[workspaceId] ?? true;
  };

  const toggleWorkspaceCollapsed = (workspaceId: string) => {
    setCollapsedWorkspaces((current) => ({ ...current, [workspaceId]: !(current[workspaceId] ?? true) }));
  };

  const expandWorkspace = (workspaceId: string) => {
    setCollapsedWorkspaces((current) => {
      if ((current[workspaceId] ?? true) === false) return current;
