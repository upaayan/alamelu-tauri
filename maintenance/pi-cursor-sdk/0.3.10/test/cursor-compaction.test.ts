...
const tool = { id: 'cursor-replay-1-1-tool-1', toolName: 'read', args: { path: 'fixture.txt' }, result: { content: [{ type: 'text' as const, text: 'fixture' }], details: {} }, isError: false };
...
describe('Cursor summary tool contract', () => {
  for (const [label, context] of [['omitted', { messages: [] }], ['empty', { messages: [], tools: [] }], ['omitted with legacy systemPrompt', { systemPrompt: 'Summarize only.', messages: [] }]] as [string, Context][]) {
    it(`routes native tools to trace when tools are ${label}`, () => {
    ...
    it(`drains tool then completed summary with tools ${label}`, async () => {
