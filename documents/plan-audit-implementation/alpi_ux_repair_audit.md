...
7. **[severity: MEDIUM]** "Existing e2e core lane green" is not a verifiable baseline in this fork, and "unit tests" has no command.
   - Where: Plan §Testing strategy — "Existing suites stay green every round: `pnpm --filter @alamelu-pi/desktop build`, unit tests, e2e core lane, driver regression suite"; checklist row "existing e2e core lane green".
   ...
   - Recommendation: replace the blanket claim with the real gate — exact commands (`node --test apps/desktop/tests/unit/*.test.mjs`, `pnpm --filter @alamelu-pi/pi-rpc-driver test`, named branded Playwright specs run via `test:e2e:runner`) and the pre-change pass/fail counts recorded once at the top of the implementation doc. Fixing the stale unbranded specs is out of scope; saying they are green is the problem.
...
7. **[MEDIUM — core lane not green / no unit command] Accepted.** Testing strategy replaced with exact gates (`node --test apps/desktop/tests/unit/*.test.mjs`, `pnpm --filter @alamelu-pi/pi-rpc-driver test`, named branded specs via `test:e2e:runner`) and a recorded baseline; unbranded-lane breakage documented as pre-existing and out of scope, not claimed green.
...
**Scope:** repair commit `d3cfb09` on top of `107d324` (verified from `.git/logs/HEAD`), against my Round 1 findings and plan rev 4. Same method as Round 1: no shell in this session, so recorded outputs were audited by reading the code they claim to describe; the live-provider repro was not re-run.

### Round-1 findings: re-verified in code, not taken on trust

- **M1 — RESOLVED.** The `runFailed` timeline row now maps: `describeError(event.error.message)` with headline as label and `described.detail ?? event.error.code` in the existing detail slot (`app-store-timeline.ts:200-206`) — exactly the plan's "extended" semantics, with a sensible code fallback when the raw is short. The failure notification body is the headline (`notification-manager.ts:130-134`). `lastErrorDetail` finally has a consumer: rendered as `error-banner__detail` under the headline (`composer-surface.tsx:219-224`), threaded `App.tsx:2461` → `composer-panel.tsx:52/104/129`, with real CSS for both new classes (`main.css:3568-3579`).
- **M2 — RESOLVED.** The warn verdict now applies the switch, syncs config, posts the "Model set to …" activity item, then surfaces `decision.reason` via `withError` (`app-store-composer.ts:437-443`) — allowed-but-warned, exactly the plan's "shows the banner and proceeds." I checked the mapping path: the warn reason (~120 chars) matches no rule and passes through `describeError`'s fallback verbatim, so the banner shows the intended sentence, not a truncation.
- **M3 — RESOLVED.** `dropdownPlacement` and `.model-selector__dropdown--below` now have zero matches anywhere in `apps/desktop`; both dropdown sites render a plain `className="model-selector__dropdown"` (`model-selector.tsx:115`, `:175`). The doc row was corrected honestly ("in the repair commit, not Phase 1 — see Round 2").
- **M4 — SUBSTANTIALLY RESOLVED (residual below).** The §3.5 spec edit is done and done well: the badge test is branded (`new-thread-composer.spec.ts:268-270`), asserts the shipped friendly label AND the raw id in `title` (`:302-303` — the title assertion is beyond what the plan asked), and its catalog assertions were adapted to the seeded pi catalog exactly per the plan's R3-2 caveat, keeping the badge-label assertion as the point of the test. The stale contradicting `:297` assertion is gone. The branded gate was actually executed and recorded: the edited test green plus all four branded provider-settings tests green. I verified the unbranded-failure claim precisely: `provider-settings.spec.ts` has eight tests, and the four failing ones (`:14`, `:295`, `:349`, `:402`) are exactly the four with no `PI_GUI_BRAND` override (branded: `:74`, `:130`, `:186`, `:236` with overrides inside at `:91/:146/:205/:269`) — the pre-existing sdk-driver condition the plan documents as not-claimed-green. The doc's account of this is accurate.
- **M5 — RESOLVED.** Rename is pending-until-confirm end to end: `ThreadSessionRow` keeps the editor open with input and Save disabled while `renamePending` (`sidebar.tsx:579-627`, `:650`, `:661`), and the chain works because `handleRenameSession` now RETURNS the promise (`App.tsx:1913-1914`) — I checked specifically that the row's `typeof result.finally` guard receives a real promise. No optimistic close-then-revert; a failure lands in the mapped banner. `SecondarySurface` gained the `lastError` slot with a testid and CSS (`secondary-surface.tsx:53-58`, `main.css:3581`), wired for Skills (`App.tsx:2177`) and Extensions (`:2219`) — plan §3.4's "renders the mapped lastError" as specified.

No new regressions found in the repair: the timeline detail-slot change only widens a display string; the warn path's double emit is benign; the rename promise can't reject through `updateSnapshot` (failures resolve into state), so the editor always settles; typecheck, build, unit (55), and driver (52) are recorded green post-repair and the changed files are consistent with those claims.

### Findings

1. **[severity: LOW]** The preventive-test tail of Round-1 M4 remains undelivered, and the repair table's M4 row reads as a full fix.
   - Where: no new unit tests (inventory unchanged: 12 files, 55 tests — §1.2's "malformed JSONL lines skipped" row and the Round-2-settled detector true-positive/true-negative rows for `readThreadStats`/`legacyNormalizedId` are still absent); none of the plan-named new specs were written (1.3 dropdown bounding-box; 1.4 worktree-attempt + double-click-Start; Phase 3 follow-up-visible + `/tree`-absent); and the four pre-existing branded `new-thread-composer` tests (`:29`, `:110`, `:161`, `:216`) are not in the recorded run (the `--grep` was scoped to the one edited test).
   - Why LOW now, when Round-1's aggregate was MEDIUM: the sharp components are fixed — the contradiction is gone, the mandated renderer change has a runnable, executed gate, and every behavior behind the missing specs is implemented and was verified by hand in these two rounds; the detector's real-world positive case is additionally evidenced by the 1.6 live-repro normalization mapping on real session ids. What remains is regression-protection debt, not hidden defect risk.
   - NOT waived: these are plan-committed deliverables. This entry is now the explicit record Round 1 asked for — owner should either schedule the specs/unit rows as a small follow-up or explicitly waive them. The repair table's M4 row should say "partially; remainder recorded in Audit Round 2."

2. **[severity: LOW]** The New-Thread banner still renders headline-only — `lastErrorDetail` was threaded to the session composer but not to `NewThreadView`'s `ComposerSurface`.
   - Where: `App.tsx` captures only `next.lastError` into `newThreadComposerError` (`:1991-1993`) and `NewThreadView` passes no `lastErrorDetail` (`new-thread-view.tsx:172-173`); plan §1.1 named both consumers ("ComposerSurface and New-Thread banner render headline + small detail line"). Two-line follow-up: capture `next.lastErrorDetail` alongside and pass it through — the surface already supports the prop.

3. **[severity: LOW]** Doc bookkeeping: the repair round claims "both LOW doc issues corrected" without saying which two, and Round-1 L6 — record in the implementation doc that the intra-phase commit granularity was consolidated (plan named eleven commits; four landed incl. the repair) and why — is still absent from the doc.
   - Where: implementation doc "Repair round" preamble; header at line 4 still lists only the three phase commits.
   - One-paragraph doc edit; no code implication.

### Verdict: PASS

Zero HIGH, zero MEDIUM, three LOW. All five Round-1 MEDIUMs are genuinely fixed in `d3cfb09` and I verified each in code, including the two easy-to-fake ones: the branded gate really ran against the really-edited spec (the assertions now match the shipped label AND add a `title` check), and the rename-pending chain is complete only because `handleRenameSession` returns the promise — the detail a cosmetic fix would have missed. The provider-settings failure account is exact: the four failing tests are precisely the four unbranded ones, the plan-documented baseline condition, unchanged by this work.

The one substantive remainder — the preventive specs and detector unit rows the plan committed — is recorded above as LOW-1 and routed to the owner: schedule as a small follow-up or waive explicitly. This PASS does not waive it; it judges that with every behavior implemented, every executed gate green, and the debt now on the record, nothing blocking remains. Findings 2 and 3 are a two-line thread-through and a one-paragraph doc note.

My involvement in this round ends here.
