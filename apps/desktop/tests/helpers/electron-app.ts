...
async function resolveExternalPiBinForTests(): Promise<string> {
  const pathCandidates = (process.env.PATH ?? "")
    .split(delimiter)
    .filter(Boolean)
    .map((entry) => join(entry, "pi"));
  const candidates = [
    process.env.PI_GUI_PI_BIN,
    ...pathCandidates,
    ...(await nvmPiCandidatesForTests()),
    resolve(homedir(), ".local/bin/pi"),
    "/opt/homebrew/bin/pi",
    "/usr/local/bin/pi",
    resolve(homedir(), ".pi/agent/bin/pi"),
  ].filter((candidate): candidate is string => Boolean(candidate?.trim()));

  for (const candidate of candidates) {
    const expanded = expandHomePath(candidate);
    const absoluteCandidate = isAbsolute(expanded) ? expanded : resolve(expanded);
    try {
  ...
  throw new Error("Unable to find installed pi binary for session fixture seeding.");
}

async function nvmPiCandidatesForTests(): Promise<string[]> {
  const versionsDir = resolve(homedir(), ".nvm", "versions", "node");
  try {
    ...
    return entries
      ...
      .map((version) => resolve(versionsDir, version, "bin", "pi"));
  } catch {
    return [];
  }
}
...
async function importExternalSessionManager(): Promise<{
  ...
  const piBin = await resolveExternalPiBinForTests();
  const resolvedPiBin = await realpath(piBin);
  const sessionManagerPath = resolve(dirname(resolvedPiBin), "core", "session-manager.js");
...
export async function createNamedThread(
  ...
  const session = await waitForSessionByTitle(window, targetWorkspaceId, title);
...
export async function createSessionViaIpc(window: Page, workspaceIdOrPath: string, title: string): Promise<void> {
  ...
  await expect
    .poll(async () => {
      const state = await getDesktopState(window);
      const workspace = state.workspaces.find((entry) => entry.id === workspaceIdOrPath || entry.path === workspaceIdOrPath);
