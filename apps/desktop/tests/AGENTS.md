...
- `pnpm --filter @alamelu-pi/desktop run test:e2e` currently runs only `core`. Use `test:e2e:all` only when you need all lanes.
...
- Prefer repo lanes over manual Computer Use. If the local Codex skill `$alamelu-pi-computer-use-smoke` is installed, use it only for release-readiness sweeps on the real installed app or for focus-hostile native surfaces where Playwright is the wrong proof shape.
