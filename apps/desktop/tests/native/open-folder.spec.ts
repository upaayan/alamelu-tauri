...
test("selecting New Workspace from the Alpi new-thread picker opens the folder dialog", async () => {
  test.setTimeout(60_000);
  const userDataDir = await makeUserDataDir();
  const initialWorkspacePath = await makeWorkspace("native-new-workspace-initial");
  const openedWorkspacePath = await makeWorkspace("native-new-workspace-picked");
  const harness = await launchDesktop(userDataDir, {
    initialWorkspaces: [initialWorkspacePath],
    testMode: "foreground",
    envOverrides: {
  ...
  try {
    ...
    await stubNextOpenDialog(harness, [openedWorkspacePath]);
    await window.locator(".new-thread__workspace").selectOption({ label: "New Workspace..." });
    ...
    await expect
      .poll(async () => {
        ...
        const pickerValue = await window.locator(".new-thread__workspace").inputValue();
        return {
      ...
      .toEqual({
        selectedPath: openedWorkspacePath,
        pickerValue: openedWorkspacePath,
      });
