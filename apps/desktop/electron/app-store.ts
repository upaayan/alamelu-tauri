...
export interface DesktopAppStoreOptions {
  ...
  readonly enableNoRepositoryWorkspace?: boolean;
...
export class DesktopAppStore implements AppStoreInternals {
  ...
  private readonly enableNoRepositoryWorkspace: boolean;
  ...
  constructor(options: DesktopAppStoreOptions) {
    ...
    if (!options.driver) {
      throw new Error("DesktopAppStore requires an explicit session driver.");
    }
  ...
  private async initializeInternal(): Promise<void> {
    ...
    try {
      ...
      if (this.enableNoRepositoryWorkspace) {
        await this.ensureNoRepositoryWorkspace();
      }
  ...
  private async ensureNoRepositoryWorkspace(): Promise<void> {
    try {
      await mkdir(this.noRepositoryWorkspacePath, { recursive: true });
      this.noRepositoryWorkspacePath = await realpath(this.noRepositoryWorkspacePath);
...
function resolveSelectedWorkspaceIdFromCatalog(
  preferredWorkspaceId: string,
  workspaces: readonly { workspaceId: string; displayName?: string; path?: string }[],
): string {
  const visibleWorkspaces = workspaces.filter(
    (workspace) => !isSystemWorkspacePathOrName(workspace.displayName) && !isSystemWorkspacePathOrName(workspace.path),
  );
  if (preferredWorkspaceId && visibleWorkspaces.some((w) => w.workspaceId === preferredWorkspaceId)) {
    return preferredWorkspaceId;
  }
  return visibleWorkspaces[0]?.workspaceId ?? workspaces[0]?.workspaceId ?? "";
}
