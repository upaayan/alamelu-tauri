...
- The signing path was reviewed and not changed or invoked during this audit. Any later package/install must use only `pnpm --filter @alamelu-pi/desktop run package:alpi:dir`, which performs the documented AWS-backed explicit-keychain unlock, identity-hash signing, no timestamp, and verification. No keychain inspection, default Electron signing, or Apple password prompt path was used.
