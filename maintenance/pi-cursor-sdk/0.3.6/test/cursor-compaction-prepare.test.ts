...
beforeEach(() => {
  vi.clearAllMocks();
  stores.setSdkOperations({ getDefaultStateRoot: () => "/tmp/cursor-compaction-fixture", openSqliteStore: async () => ({ dispose: async () => {} }) as never });
...
afterEach(async () => { await releaseAllPendingCursorLiveRunsForTests(); await resetSessionCursorAgent(); stores.setSdkOperations(undefined); });
